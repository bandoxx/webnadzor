...
#[Route(path: '/api/{clientId}/device/{deviceId}', methods: 'POST', name: 'api_device_delete')]
class DeviceDeleteController extends AbstractController
{

    public function __invoke($clientId, $deviceId, Request $request, DeviceRepository $deviceRepository, PurgeDeviceData $purgeDeviceData, UserPasswordHasherInterface $userPasswordChecker)
    {
        $user = $this->getUser();
