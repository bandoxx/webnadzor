...
#[AsCommand(
    name: 'app:history:device-data-archiver',
    description: 'Command for making XLSX and PDF archives on daily and monthly level.',
)]
class HistoryDeviceDataArchiver extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $device = $this->deviceRepository->find($input->getArgument('deviceId'));

        if (!$device) {
            return Command::FAILURE;
        }

        $fromDate = new \DateTime($input->getArgument('fromDate'));
        $toDate = new \DateTime($input->getArgument('toDate'));
